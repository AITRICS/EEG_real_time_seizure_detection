# Copyright (c) 2022, Seyun Kim. All rights reserved.
#
...
class TemperatureScaling(nn.Module):
    ...
    def set_temperature(self, valid_loader):
        ...
        def eval():
            optimizer.zero_grad()
            loss = nll_criterion(self.temperature_scale(logits), labels) # temperature로 나눈 값이 label과 비슷해지도록. 따라서 그냥 logit값은 0.99, 0.01, .. 이런 식이 아님
            loss.backward()
            return loss
        optimizer.step(eval)
