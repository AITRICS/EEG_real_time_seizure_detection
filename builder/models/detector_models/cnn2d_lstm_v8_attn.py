...
class CNN2D_LSTM_V8_ATTN(nn.Module):
        def __init__(self, args, device):
                super(CNN2D_LSTM_V8_ATTN, self).__init__()      
                self.args = args
                ...
                self.agvpool1 = nn.AdaptiveAvgPool2d((20,1))
                self.all_agvpool = nn.AdaptiveAvgPool2d((1,1))
                ...
                self.classifier = nn.Sequential(
                        nn.Linear(in_features=10240, out_features= 128, bias=True),
                        nn.BatchNorm1d(128),
                        self.activations[activation],
                        nn.Linear(in_features=128, out_features= args.output_dim, bias=True),
                )
        ...
        def forward(self, x):
                ...
                x_all = self.all_agvpool(x).squeeze(2).permute(0,2,1)
                x_1 = self.agvpool1(x).squeeze(3)
                self.hidden = tuple(([Variable(var.data) for var in self.hidden]))
                attn1 = torch.bmm(self.hidden[0][0].unsqueeze(1), x_1)
                attn2 = torch.bmm(self.hidden[0][1].unsqueeze(1), x_1)
                _ , self.hidden = self.lstm(x_all, self.hidden)    

                attn_v1 = F.softmax(attn1, dim=0)
                attn_v2 = F.softmax(attn2, dim=0)
                weighted_attn1 = torch.mul(x_1, attn_v1)
                weighted_attn2 = torch.mul(x_1, attn_v2)
                weighted_attn_cat = torch.cat((weighted_attn1, weighted_attn2), 1)
                output = weighted_attn_cat.reshape(weighted_attn_cat.size(0), -1)
